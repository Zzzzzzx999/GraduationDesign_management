        ...
        <el-form :model="newStationForm" ref="addStationRef" :inline="true" label-width="100px" label-position="top">
          <el-row>
            <el-form-item label="主题背景" required prop="cover_image">
              <div>
                <img v-if="!newStationForm.cover_image" class="the_img border" src="../../assets/images/avatar.jpg" alt="" @click="chooseImg" />
                <img v-else class="the_img border" :src="newStationForm.cover_image" alt="" @click="chooseImg">
                <div class="btn-box">
          ...
          <el-row>
            ...
            <el-form-item label="电台类型" required prop="cateName">
              <el-select filterable v-model="newStationForm.cateName" placeholder=" " clearable>
                <el-option v-for="item in cateNames" :key="item.alias" :label="item.name"
                  :value="item.name">
                </el-option>
              </el-select>
            </el-form-item>
...
<script>
import { updateStationAPI, getStationInfoAPI, getStationCates} from '@/api/radioStation'
export default {
  name: 'newUser',
  data () {
    return {
    ...
    if (this.$route.query.id) {
      ...
      setTimeout(() => {
        getStationInfoAPI(this.userid).then((res) => {
          ...
          console.log('this.newStationForm',this.newStationForm)
  ...
  methods: {
    ...
    submitFn () {
      this.$refs.addStationRef.validate(async valid => {
        if (valid) {
          const { data: res } = await updateStationAPI(this.newStationForm)
          if (res.status == 1 && res.message == 'request entity too large') return this.$message.error('换张小点的图片试试？')
          if (res.status == 1 && res.message == '电台名被占用，请更换其他电台名！') return this.$message.error(res.message)
          if (res.status !== 0) return this.$message.error('修改电台失败, ( '+res.message+' )')
